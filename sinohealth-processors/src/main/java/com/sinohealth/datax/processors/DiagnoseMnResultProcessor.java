 ...
 * @date 20220905
 **/
public class DiagnoseMnResultProcessor implements Processor<RegCheck, StandardMnCheckRecordList> {

    public static final Logger logger = LoggerFactory.getLogger(DiagnoseMnResultProcessor.class);
    ...
    @Override
    public StandardMnCheckRecordList dataProcess(RegCheck check, StandardMnCheckRecordList list, CommonData commonData) {
        ArrayList<StandardMnCheckRecord> recordList = new ArrayList<>();
        ...
        for (String s : summaryArr) {
            List<StandardMnCheckRecord> standardDiagnoseRecords = logicJudgeInspection(s, commonData.getBasItemAliasList(), check);
        ...
        //对于相同疾病去重处理
        Map<String, List<StandardMnCheckRecord>> showNameMap = recordList.stream().collect(Collectors.groupingBy(StandardMnCheckRecord::getItemName));
        for (String showName : showNameMap.keySet()) {
            List<StandardMnCheckRecord> temp = showNameMap.get(showName);
            // 根据等级排序
            ArrayList<String> listS = new ArrayList<>();
            int i = 0;
            HashMap<String, StandardMnCheckRecord> hashMapDisease = new HashMap<>();
            for (StandardMnCheckRecord drnd : temp) {
    ...
    private List<StandardMnCheckRecord> logicJudgeInspection(String inspectionItem, List<BasItemAlias> allItemAlias, RegCheck check) {
        /**
    ...
    private  List<StandardMnCheckRecord> preProcessItemResult(String itemName, String itemResults, RegCheck check) {

        List<StandardMnCheckRecord> list = new ArrayList<>();
        /*
    ...
    private List<StandardMnCheckRecord> addRelateDisease(String itemName, String clause, RegCheck check) {
        ...
        if (!mostSimuList.isEmpty()) {
            ...
            for (BasInspectionKeyword inspectionKeyword : removeContainsList) {
                StandardMnCheckRecord record = new StandardMnCheckRecord();
                record.setVid(check.getVid());
                record.setResults(inspectionKeyword.getDiseaseSigns());
                record.setItemName(inspectionKeyword.getShowName());
                record.setCleanStatus(1);
                record.setCleanTime(new Date());
                record.setItemFt(itemName);
                record.setResultsDiscrete(2);
                record.setImageDescribe(clause);
                list.add(record);
            }
        }
        return list;
