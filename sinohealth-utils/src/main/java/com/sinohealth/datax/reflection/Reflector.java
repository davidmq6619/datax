package com.sinohealth.datax.reflection;

import com.sinohealth.datax.reflection.annotations.db_alias;
import com.sinohealth.datax.reflection.invoker.GetFieldInvoker;
import com.sinohealth.datax.reflection.invoker.Invoker;
import com.sinohealth.datax.reflection.invoker.MethodInvoker;
import com.sinohealth.datax.reflection.invoker.SetFieldInvoker;
import com.sinohealth.datax.reflection.property.PropertyNamer;
...
public class Reflector {
	...
	// 数据库字段是否可以更新
	private Map<String, Boolean> field_ignore = new HashMap<String, Boolean>();
	...
	private Reflector(Class<?> clazz) {
		type = clazz;
		// 处理类的名称与别名
		db_alias alias = clazz.getAnnotation(db_alias.class);
		if (alias != null) {
	...
	private void addFields(Class<?> clazz) {
		...
		for (Field field : fields) {
			...
			// 处理db_alias注解
			db_alias alias = field.getAnnotation(db_alias.class);
			if (alias != null) {
				alias_name.put(alias.value(), field.getName());
				name_alias.put(field.getName(), alias.value());
			}
		}
	...
	public boolean fieldIgnore(String name) {
		Boolean canchange = field_ignore.get(name);
		if (canchange != null) {
			return canchange;
		} else {
			return false;
	...
	public List<String> getAllColumns() {
		...
		if (!caseInsensitivePropertyMap.isEmpty()) {
			Set<Map.Entry<String,String>> entrySet = caseInsensitivePropertyMap.entrySet();
			for (Map.Entry<String,String> entry : entrySet) {
				String lowerCaseFieldName = entry.getValue();
				String columnName = getColumn(lowerCaseFieldName);
